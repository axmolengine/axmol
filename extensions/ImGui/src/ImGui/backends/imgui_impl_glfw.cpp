...
//  'io.ConfigFlags |= ImGuiConfigFlags_ViewportsEnable'. [X] Multiple Dear ImGui contexts support.
...
//  ImGuiMouseCursor_Progress cursors. [ ] Multi-viewport: ParentViewportID not honored, and so
...
//  2025-07-08: Made ImGui_ImplGlfw_GetContentScaleForWindow(), ImGui_ImplGlfw_GetContentScaleForMonitor() helpers
//  return 1.0f on Emscripten and Android platforms, matching macOS logic. (#8742, #8733) 2025-06-18: Added support for
//  multiple Dear ImGui contexts. (#8676, #8239, #8069) 2025-06-11: Added
//  ImGui_ImplGlfw_GetContentScaleForWindow(GLFWwindow* window) and
//  ImGui_ImplGlfw_GetContentScaleForMonitor(GLFWmonitor* monitor) helper to facilitate making DPI-aware apps.
//  2025-05-15: [Docking] Add Platform_GetWindowFramebufferScale() handler, to allow varying Retina display density on
//  multiple monitors. 2025-04-26: [Docking] Disable multi-viewports under Wayland. (#8587) 2025-03-10: Map
//  GLFW_KEY_WORLD_1 and GLFW_KEY_WORLD_2 into ImGuiKey_Oem102. 2025-03-03: Fixed clipboard handler assertion when using
//  GLFW <= 3.2.1 compiled with asserts enabled. 2025-02-21: [Docking] Update monitors and work areas information every
//  frame, as the later may change regardless of monitor changes. (#8415) 2024-11-05: [Docking] Added Linux workaround
//  for spurious mouse up events emitted while dragging and creating new viewport. (#3158, #7733, #7922) 2024-08-22:
...
static void ImGui_ImplGlfw_GetWindowSizeAndFramebufferScale(GLFWwindow* window,
                                                            ImVec2* out_size,
                                                            ImVec2* out_framebuffer_scale)
{
    ...
    if (out_framebuffer_scale != nullptr)
        *out_framebuffer_scale =
            (w > 0 && h > 0) ? ImVec2((float)display_w / (float)w, (float)display_h / (float)h) : ImVec2(1.0f, 1.0f);
}
...
static void ImGui_ImplGlfw_RenderWindow(ImGuiViewport* viewport, void*)
{
    ...
    {
        // axmol spec
        ImGui_ImplAxmol_PostCommand([vd] { ImGui_ImplAxmol_MakeCurrent(vd->Window); });
    }
}
