...
struct IMGUI_API ImGuiInputTextState
{
    ...
    ImVector<char> TextToRevertTo;  // value to revert to when pressing Escape = backup of end-user buffer at the time
...
struct IMGUI_API ImGuiMultiSelectTempData
{
    ...
    bool IsKeyboardSetRange;  // Set by BeginMultiSelect() when using Shift+Navigation. Because scrolling may be
                              // affected we can't afford a frame of lag with Shift+Navigation.
    bool NavIdPassedBy;
...
enum ImGuiDockNodeFlagsPrivate_
{
    ...
    ImGuiDockNodeFlags_NoDockingOverEmpty = 1 << 22,  //       // Disable this node from being docked over an empty node
...
struct ImGuiViewportP : public ImGuiViewport
{
    ...
    int LastFocusedStampCount;  // Last stamp number from when a window hosted by this viewport was focused (by
                                // comparing this value between two viewport we have an implicit viewport z-order we use
                                // as fallback)
    ...
    bool LastFocusedHadNavWindow;  // Instead of maintaining a LastFocusedWindow (which may harder to correctly
                                   // maintain), we merely store weither NavWindow != NULL last time the viewport was
                                   // focused.
...
struct ImGuiContext
{
    ...
    ImGuiWindow* HoveredWindowBeforeClear;  // Window the mouse is hovering. Filled even with _NoMouse. This is
    ...
    ImGuiID NavFocusScopeId;    // Focused focus scope (e.g. selection code often wants to "clear other items" when
...
enum ImGuiTabItemFlagsPrivate_
{
    ...
    ImGuiTabItemFlags_Unsorted  = 1 << 23,  // [Docking] Trailing tabs with the _Unsorted flag will be sorted based on
...
struct IMGUI_API ImGuiTable
{
    ...
    ImS16 InstanceCurrent;  // Count of BeginTable() calls with same ID in the same frame (generally 0). This is a
                            // little bit similar to BeginCount for a window, but multiple tables with the same ID are
                            // multiple tables, they are just synced.
...
#    ifndef IMGUI_DISABLE_OBSOLETE_FUNCTIONS
inline void RenderNavHighlight(const ImRect& bb,
                               ImGuiID id,
...
// FocusableItemRegister(): Refactored focus/nav/tabbing system in 1.82 and 1.84. If you have old/custom copy-and-pasted
...
IMGUI_API ImFontBaked* ImFontAtlasBakedGetClosestMatch(ImFontAtlas* atlas,
                                                       ImFont* font,
                                                       float font_size,
                                                       float font_rasterizer_density);
...
IMGUI_API ImFontGlyph* ImFontAtlasBakedAddFontGlyph(ImFontAtlas* atlas,
                                                    ImFontBaked* baked,
                                                    ImFontConfig* src,
...
IMGUI_API void ImFontAtlasBakedDiscardFontGlyph(ImFontAtlas* atlas,
                                                ...
                                                ImFontBaked* baked,
                                                ImFontGlyph* glyph);
IMGUI_API void ImFontAtlasBakedSetFontGlyphBitmap(ImFontAtlas* atlas,
                                                  ImFontBaked* baked,
                                                  ImFontConfig* src,
                                                  ImFontGlyph* glyph,
                                                  ImTextureRect* r,
...
IMGUI_API ImFontAtlasRectId ImFontAtlasPackAddRect(ImFontAtlas* atlas,
                                                   int w,
                                                   int h,
