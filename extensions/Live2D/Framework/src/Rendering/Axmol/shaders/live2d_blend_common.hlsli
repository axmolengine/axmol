...
float CubismGetMax(float3 rgbC)
{
    return max(rgbC.r, max(rgbC.g, rgbC.b));
}

float CubismGetMin(float3 rgbC)
{
    return min(rgbC.r, min(rgbC.g, rgbC.b));
}

float CubismSaturation(float3 rgbC)
{
    return CubismGetMax(rgbC) - CubismGetMin(rgbC);
}

float CubismLuma(float3 rgbC)
{
    return 0.30 * rgbC.r + 0.59 * rgbC.g + 0.11 * rgbC.b;
}

float3 CubismClipColor(float3 rgbC)
{
...
float3 CubismSetLuma(float3 rgbC, float luma)
{
    return CubismClipColor(rgbC + (luma - CubismLuma(rgbC)));
}

float3 CubismSetSaturation(float3 rgbC, float saturation)
{
    float maxv = CubismGetMax(rgbC);
    float minv = CubismGetMin(rgbC);
...
float3 ColorBlend(float3 colorSource, float3 colorDestination, int colorBlendMode)
{
    if (colorBlendMode == 1)
    {
    ...
    if (colorBlendMode == 6)
    {
        return max(float3(0.0, 0.0, 0.0), colorSource + colorDestination - 1.0);
    }
    if (colorBlendMode == 7)
    {
...
float4 AlphaBlend(float3 color, float4 colorSource, float4 colorDestination, int alphaBlendMode)
{
    float3 parameter;
    if (alphaBlendMode == 1)
    {
        parameter = float3(colorSource.a * colorDestination.a, 0.0, colorDestination.a * (1.0 - colorSource.a));
    }
    else if (alphaBlendMode == 2)
    {
        parameter = float3(0.0, 0.0, colorDestination.a * (1.0 - colorSource.a));
    }
    else if (alphaBlendMode == 3)
    {
        parameter = float3(min(colorSource.a, colorDestination.a), max(colorSource.a - colorDestination.a, 0.0), max(colorDestination.a - colorSource.a, 0.0));
    }
    else if (alphaBlendMode == 4)
    {
        parameter = float3(max(colorSource.a + colorDestination.a - 1.0, 0.0), min(colorSource.a, 1.0 - colorDestination.a), min(colorDestination.a, 1.0 - colorSource.a));
    }
    else
    {
        parameter = float3(colorSource.a * colorDestination.a, colorSource.a * (1.0 - colorDestination.a), colorDestination.a * (1.0 - colorSource.a));
    }
