...
int lua_cocos2dx_csloader_CSLoader_bindCallback(lua_State* tolua_S)
{
    ...
    if (argc == 4) 
    {
        ...
        ok &= luaval_to_object<axis::ui::Widget>(tolua_S, 4, "ccui.Widget",&arg2, "cc.CSLoader:bindCallback");

        ok &= luaval_to_object<axis::Node>(tolua_S, 5, "cc.Node",&arg3, "cc.CSLoader:bindCallback");
        if(!ok)
        {
...
int lua_cocos2dx_csloader_CSLoader_getInstance(lua_State* tolua_S)
{
    ...
    if (argc == 0)
    {
        ...
        object_to_luaval<axis::CSLoader>(tolua_S, "cc.CSLoader",(axis::CSLoader*)ret);
        return 1;
    }
