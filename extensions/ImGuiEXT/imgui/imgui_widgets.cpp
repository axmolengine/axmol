                       ...
                       ImGuiSliderFlags flags)
{
    ...
    if (!temp_input_is_active)
    {
        const bool focus_requested =
            temp_input_allowed && (g.LastItemData.StatusFlags & ImGuiItemStatusFlags_Focused) != 0;
                        ...
                        ImGuiInputTextFlags flags)
{
    ...
    flags |= ImGuiInputTextFlags_NoMarkEdited;  // We call MarkItemEdited() ourselves by comparing the actual data
                                     ...
                                     ImGuiInputSource input_source)
{
                                         ...
                                         ImGuiInputTextFlags_CharsScientific)))
    {
        ...
        // output/input of printf/scanf. The standard mandate that programs starts in the "C" locale where the decimal
        // point is '.'. We don't really intend to provide widespread support for it, but out of empathy for people
                        ...
                        void* callback_user_data)
{
    ...
    const bool user_nav_input_start =
        (g.ActiveId != id) &&
        ((g.NavInputId == id) || (g.NavActivateId == id && g.NavInputSource == ImGuiInputSource_Keyboard));
    ...
    if (g.ActiveId == id)
    {
        ...
        if (apply_new_text)
        {
            // We cannot test for 'backup_current_text_length != apply_new_text_length' here because we have no
            // guarantee that the size of our owned buffer matches the size of the string object held by the user, and
            // by design we allow InputText() to be used without any storage on user's side.
...
bool ImGui::BeginMenu(const char* label, bool enabled)
{
    ...
    else
    {
        ...
        float icon_w = 0.0f;  // FIXME: This not currently exposed for BeginMenu() however you can call
                              // window->DC.MenuColumns.DeclColumns(w, 0, 0, 0) yourself
                          ...
                          ImGuiDockNode* dock_node)
{
    ...
    if ((flags & ImGuiTabBarFlags_Reorderable) != (tab_bar->Flags & ImGuiTabBarFlags_Reorderable) ||
        ...
        if (tab_bar->Tabs.Size > 1 &&
            (flags & ImGuiTabBarFlags_DockNode) == 0)  // FIXME: TabBar with DockNode can now be hybrid
