...
class Cocos2dIniParser:
    def __init__(self):
        ...
        self._cp.read(os.path.join(self.cocos2d_path, "axys.ini"))

        # XXX: override with local config ??? why ???
        # self._cp.read("~/.cocos2d-js/axys.ini")
...
class DataStatistic(object):
    '''
    In order to improve axys, we periodically send anonymous data about how you use axys.
    You can turn off this function by change the value of "enable_stat" in axys.ini.

    Information collected will be used to develop new features and improve axys.
    ...
    def get_cfg_file_path(cls):
        return os.path.join(os.path.expanduser('~/.axys'), 'local_cfg.json')
    ...
    def change_agree_stat(cls, agreed):
        ...
        ini_file = os.path.join(get_current_path(), "axys.ini")
        f = open(ini_file)
    ...
    def init_stat_obj(cls):
        if cls.inited == False:
            # get the axys_stat module
            m = None
            try:
                m = __import__("axys_stat")
            except:
                pass
...
class CCPlugin(object):
    ...
    def get_cocos2d_path(cls):
        """returns the path where axys is installed"""
        ...
        # /Users/myself/axys/tools/console/bin
    ...
    def get_templates_paths(cls):
        ...
        #
        user_path = os.path.expanduser("~/.axys/templates")
    ...
    @staticmethod
    def _log_path():
        log_dir = os.path.expanduser("~/.axys")
        if not os.path.exists(log_dir):
            os.mkdir(log_dir)
        return os.path.join(log_dir, "axys.log")
    ...
    def init(self, args):
        ...
        self._platforms = axys_project.Platforms(self._project, args.platform, args.proj_dir)
    ...
    def parse_args(self, argv):
        ...
        platform_list = axys_project.Platforms.list_for_display()
        ...
        if args.src_dir is None:
            self._project = axys_project.Project(os.path.abspath(os.getcwd()))
        else:
            self._project = axys_project.Project(
        ...
        if args.listplatforms and self._project is not None:
            platforms = axys_project.Platforms(self._project, args.platform, args.proj_dir)
...
if __name__ == "__main__":
    ...
    if force_disable_stats:
        ...
        DataStatistic.stat_event('axys', 'start', 'invoked')
    ...
    try:
        ...
        if command in plugins:
            DataStatistic.stat_event('axys', 'running_command', command)
