...
class AndroidBuilder(object):
    ...
    def __init__(self, verbose, app_android_root, no_res, proj_obj, mode, build_type, app_abi, gradle_support_ndk=False):
        ...
        self.sdk_root = axys.check_environment_variable('ANDROID_SDK_ROOT')
        self.ant_root = None
        if os.path.exists(os.path.join(self.app_android_root, "gradle.properties")):
            ...
            raise axys.CCPluginError(MultiLanguage.get_string('COMPILE_GRADLE_PROPERTIES_NOT_FOUND'),
                                  axys.CCPluginError.ERROR_PATH_NOT_FOUND)
        self._parse_cfg()

    def _run_cmd(self, command, cwd=None):
        axys.CMDRunner.run_cmd(command, self._verbose, cwd=cwd)
    ...
    def _parse_cfg(self):
        ...
        try:
            ...
            raise axys.CCPluginError(MultiLanguage.get_string('COMPILE_ERROR_PARSE_CFG_FAILED_FMT', self.cfg_path),
                                      axys.CCPluginError.ERROR_PARSE_FILE)

        if axys.dict_contains(cfg, project_compile.CCPluginCompile.CFG_KEY_MUST_COPY_RESOURCES):
            if self._no_res:
        ...
        move_cfg = {}
        self.key_store = None
        if axys.dict_contains(cfg, AndroidBuilder.CFG_KEY_STORE):
        ...
        if axys.dict_contains(cfg, AndroidBuilder.CFG_KEY_STORE_PASS):
        ...
        self.alias = None
        if axys.dict_contains(cfg, AndroidBuilder.CFG_KEY_ALIAS):
        ...
        if axys.dict_contains(cfg, AndroidBuilder.CFG_KEY_ALIAS_PASS):
    ...
    def _get_android_sdk_tools_ver(self, sdk_tools_path):
        ...
        raise axys.CCPluginError(MultiLanguage.get_string('COMPILE_ERROR_UNKNOWN_ANDROID_SDK_TOOLS_VERSION'),
                                  axys.CCPluginError.ERROR_PATH_NOT_FOUND)
    ...
    def _write_local_properties(self, folder_path):
        ...
        # ndk_dir = axys.check_environment_variable('ANDROID_NDK')
        if axys.os_is_win32():
    ...
    def do_ndk_build(self, ndk_build_param, mode, build_type, compile_obj):
        axys.Logging.info(MultiLanguage.get_string('COMPILE_INFO_NDK_BUILD_TYPE', build_type))
        ndk_root = axys.check_environment_variable('ANDROID_NDK')
        ...
        ndk_path = axys.CMDRunner.convert_path_to_cmd(os.path.join(ndk_root, "ndk-build"))
        ...
        for line in open(property_file):
            ...
            if match is not None:
                ...
                if os.path.isdir(abs_lib_path):
                    ...
                    command = "%s update lib-project -p %s -t %s" % (axys.CMDRunner.convert_path_to_cmd(sdk_tool_path), abs_lib_path, target_str)
    ...
    def get_api_level(self, target_str, raise_error=True):
        ...
        if match is not None:
            ...
            if raise_error:
                raise axys.CCPluginError(MultiLanguage.get_string('COMPILE_ERROR_NOT_VALID_AP_FMT', target_str),
                                          axys.CCPluginError.ERROR_PARSE_FILE)
            else:
                ret = -1
    ...
    def get_target_config(self, proj_path):
        ...
        if not os.path.isfile(property_file):
            raise axys.CCPluginError(MultiLanguage.get_string('COMPILE_ERROR_FILE_NOT_FOUND_FMT', property_file),
                                      axys.CCPluginError.ERROR_PATH_NOT_FOUND)
        ...
        raise axys.CCPluginError(MultiLanguage.get_string('COMPILE_ERROR_TARGET_NOT_FOUND_FMT', property_file),
                                  axys.CCPluginError.ERROR_PARSE_FILE)
    ...
    def check_android_platform(self, sdk_root, android_platform, proj_path):
        ...
        ret_path = os.path.join(axys.CMDRunner.convert_path_to_python(sdk_root), "platforms", ret)
        if not os.path.isdir(ret_path):
            raise axys.CCPluginError(MultiLanguage.get_string('COMPILE_ERROR_NO_AP_IN_SDK_FMT', ret),
                                      axys.CCPluginError.ERROR_PATH_NOT_FOUND)
    ...
    def gradle_build_apk(self, mode, android_platform, compile_obj):
        ...
        if compile_sdk_ver is not None:
            ...
            if not os.path.isdir(check_path):
                axys.Logging.warning(MultiLanguage.get_string('COMPILE_WARNING_COMPILE_SDK_FMT',
        ...
        if build_tools_ver is not None:
            ...
            if not os.path.isdir(check_path):
                axys.Logging.warning(MultiLanguage.get_string('COMPILE_WARNING_BUILD_TOOLS_FMT',
        ...
        if not os.path.isfile(gradle_path):
            raise axys.CCPluginError(MultiLanguage.get_string('COMPILE_ERROR_GRALEW_NOT_EXIST_FMT', gradle_path),
                                      axys.CCPluginError.ERROR_PATH_NOT_FOUND)
    ...
    def do_build_apk(self, mode, no_apk, no_sign, output_dir, custom_step_args, android_platform, compile_obj):
            ...
            if self._project._is_lua_project():
                ...
                if build_arch == self.LuaBuildArch.ONLY_BUILD_64BIT:
                    ...
                    if is_compiled:
                        ...
                        shutil.rmtree(os.path.join(src_dir, 'axys'))
        ...
        if not no_apk:
            ...
            if output_dir:
                ...
                axys.Logging.info(MultiLanguage.get_string('COMPILE_INFO_MOVE_APK_FMT', output_dir))
                ...
                raise axys.CCPluginError(MultiLanguage.get_string('COMPILE_ERROR_NOT_SPECIFY_OUTPUT'),
    ...
    def _gather_sign_info(self):
        ...
        while True:
            ...
            if os.path.isfile(abs_path):
                ...
                axys.Logging.warning(MultiLanguage.get_string('COMPILE_INFO_NOT_A_FILE'))
