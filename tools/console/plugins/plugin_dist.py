...
# ----------------------------------------------------------------------------
'''
"dist" plugin for axis command line tool
'''
...
class CCPluginDist(axis.CCPlugin):
    """
    ...
    def init(self, options, working_dir):
        ...
        if self._platforms.is_ios_active():
            if not options.provisioning:
                raise axis.CCPluginError("Provisioning profile is needed")
            else:
    ...
    def dist_android(self):
        ...
        raise axis.CCPluginError("unimplemented")
    ...
    def _find_ios_scheme(self, project_dir):
        ...
        if match is None:
            raise axis.CCPluginError("Couldn't find the schemes list")

        schemes = match.group(1).split()
        if len(schemes) == 0:
            raise axis.CCPluginError("Couldn't find a scheme")
