...
local function initWithLayer(layer)
	...
	grossini:setPosition(ax.p(size.width / 2, size.height / 3))
	tamara:setPosition(ax.p(size.width / 2, 2 * size.height / 3))
	kathia:setPosition(ax.p(size.width / 2, size.height / 2))
...
local function centerSprites(numberOfSprites)
	...
	elseif numberOfSprites == 2 then
		kathia:setPosition(ax.p(size.width / 3, size.height / 2))
		tamara:setPosition(ax.p(2 * size.width / 3, size.height / 2))
		grossini:setVisible(false)
	elseif numberOfSprites == 3 then
		grossini:setPosition(ax.p(size.width / 2, size.height / 2))
		tamara:setPosition(ax.p(size.width / 4, size.height / 2))
		kathia:setPosition(ax.p(3 * size.width / 4, size.height / 2))
	end
end
...
local function alignSpritesLeft(numberOfSprites)
	if numberOfSprites == 1 then
		tamara:setVisible(false)
		kathia:setVisible(false)
		grossini:setPosition(ax.p(60, size.height / 2))
	elseif numberOfSprites == 2 then
		kathia:setPosition(ax.p(60, size.height / 3))
		tamara:setPosition(ax.p(60, 2 * size.height / 3))
		grossini:setVisible(false)
	elseif numberOfSprites == 3 then
		grossini:setPosition(ax.p(60, size.height / 2))
		tamara:setPosition(ax.p(60, 2 * size.height / 3))
...
local function ActionManual()
	...
	tamara:setPosition(ax.p(100, 70))
...
local function ActionMove()
	...
	centerSprites(3)
	local actionBy = ax.MoveBy:create(2, ax.p(80, 80))
	local actionByBack = actionBy:reverse()

	tamara:runAction(ax.MoveTo:create(2, ax.p(size.width - 40, size.height - 40)))
	grossini:runAction(ax.Sequence:create(actionBy, actionByBack))
	kathia:runAction(ax.MoveTo:create(1, ax.p(40, 40)))
...
local function ActionScale()
	...
	local actionTo = ax.ScaleTo:create(2.0, 0.5)
	local actionBy = ax.ScaleBy:create(2.0, 1.0, 10.0)
	local actionBy2 = ax.ScaleBy:create(2.0, 5.0, 1.0)

	grossini:runAction(actionTo)
	tamara:runAction(ax.Sequence:create(actionBy, actionBy:reverse()))
	kathia:runAction(ax.Sequence:create(actionBy2, actionBy2:reverse()))
...
local function ActionRotate()
    ...
    local actionTo = ax.RotateTo:create( 2, 45)
    local actionTo2 = ax.RotateTo:create( 2, -45)
    local actionTo0 = ax.RotateTo:create(2 , 0)
    tamara:runAction(ax.Sequence:create(actionTo, actionTo0))

    local actionBy = ax.RotateBy:create(2 , 360)
    local actionByBack = actionBy:reverse()
    grossini:runAction(ax.Sequence:create(actionBy, actionByBack))

    local action0Retain = ax.RotateTo:create(2 , 0)

    kathia:runAction(ax.Sequence:create(actionTo2, action0Retain))
...
local function ActionSkew()
    ...
    local actionTo = ax.SkewTo:create(2, 37.2, -37.2)
    local actionToBack = ax.SkewTo:create(2, 0, 0)
    local actionBy = ax.SkewBy:create(2, 0.0, -90.0)
    local actionBy2 = ax.SkewBy:create(2, 45.0, 45.0)
    local actionByBack = actionBy:reverse()

    tamara:runAction(ax.Sequence:create(actionTo, actionToBack))
    grossini:runAction(ax.Sequence:create(actionBy, actionByBack))
    kathia:runAction(ax.Sequence:create(actionBy2, actionBy2:reverse()))
...
local function ActionRotationalSkewVSStandardSkew()
    ...
    box:setPosition(ax.p(s.width/2, s.height - 100 - box:getContentSize().height/2));
    layer:addChild(box);
    local label = ax.Label:createWithTTF("Standard axmol Skew", s_markerFeltFontPath, 16);
    label:setAnchorPoint(ax.p(0.5, 0.5))
    label:setPosition(ax.p(s.width/2, s.height - 100 + label:getContentSize().height));
    layer:addChild(label);
    local actionTo = ax.SkewBy:create(2, 360, 0);
    local actionToBack = ax.SkewBy:create(2, -360, 0);
    local seq = ax.Sequence:create(actionTo, actionToBack)
    ...
    box:setContentSize(boxSize);
    box:setIgnoreAnchorPointForPosition(false);
    box:setPosition(ax.p(s.width/2, s.height - 250 - box:getContentSize().height/2));
    layer:addChild(box);
    label = ax.Label:createWithTTF("Rotational Skew", s_markerFeltFontPath, 16);
    label:setAnchorPoint(ax.p(0.5, 0.5))
    label:setPosition(ax.p(s.width/2, s.height - 250 + label:getContentSize().height/2));
    layer:addChild(label);
    local actionTo2 = ax.RotateBy:create(2, 360, 0);
    local actionToBack2 = ax.RotateBy:create(2, -360, 0);
    seq = ax.Sequence:create(actionTo2, actionToBack2)
    box:runAction(seq);
...
local function ActionSkewRotate()
    ...
    uR:setAnchorPoint(ax.p(0, 0))
    layer:addChild(box)

    local actionTo = ax.SkewTo:create(2, 0, 2)
    local rotateTo = ax.RotateTo:create(2, 61.0)
    local actionScaleTo = ax.ScaleTo:create(2, -0.44, 0.47)

    local actionScaleToBack = ax.ScaleTo:create(2, 1.0, 1.0)
    local rotateToBack = ax.RotateTo:create(2, 0)
    local actionToBack = ax.SkewTo:create(2, 0, 0)

    box:runAction(ax.Sequence:create(actionTo, actionToBack))
    box:runAction(ax.Sequence:create(rotateTo, rotateToBack))
    box:runAction(ax.Sequence:create(actionScaleTo, actionScaleToBack))
...
local function ActionJump()
    ...
    local actionTo = ax.JumpTo:create(2, ax.p(300,300), 50, 4)
    local actionBy = ax.JumpBy:create(2, ax.p(300,0), 50, 4)
    local actionUp = ax.JumpBy:create(2, ax.p(0,0), 80, 4)
    local actionByBack = actionBy:reverse()

    tamara:runAction(actionTo)
    grossini:runAction(ax.Sequence:create(actionBy, actionByBack))
    kathia:runAction(ax.RepeatForever:create(actionUp))
...
local function ActionCardinalSpline()
	...
	local action = ax.CardinalSplineBy:create(3, array, 0)
	local reverse = action:reverse()
	local seq = ax.Sequence:create(action, reverse)

	tamara:setPosition(ax.p(50, 50))
	tamara:runAction(seq)

	local action2 = ax.CardinalSplineBy:create(3, array, 1)
	local reverse2 = action2:reverse()
	local seq2 = ax.Sequence:create(action2, reverse2)

	kathia:setPosition(ax.p(size.width / 2, 50))
...
local function ActionCatmullRom()
    ...
    local array = {
           ax.p(0, 0),
           ax.p(80, 80),
           ax.p(size.width - 80, 80),
           ax.p(size.width - 80, size.height - 80),
           ax.p(80, size.height - 80),
           ax.p(80, 80),
           ax.p(size.width / 2, size.height / 2),
    }

    local action = ax.CatmullRomBy:create(3, array)
    local reverse = action:reverse()
    local seq = ax.Sequence:create(action, reverse)
	tamara:runAction(seq)
    ...
    local action2 = ax.CatmullRomTo:create(3, array2)
    local reverse2 = action2:reverse()
    local seq2 = ax.Sequence:create(action2, reverse2)
    kathia:runAction(seq2)
    local drawNode1 = ax.DrawNode:create()
...
local function ActionBezier()
    ...
    bezier.controlPoint_1 = ax.p(0, size.height / 2)
    bezier.controlPoint_2 = ax.p(300, - size.height / 2)
    bezier.endPosition = ax.p(300, 100)
    ]]--
    local bezier = {
        ax.p(0, size.height / 2),
        ax.p(300, - size.height / 2),
        ax.p(300, 100),
    }
    local bezierForward = ax.BezierBy:create(3, bezier)
    local bezierBack = bezierForward:reverse()
    local rep = ax.RepeatForever:create(ax.Sequence:create(bezierForward, bezierBack))

    -- sprite 2
    tamara:setPosition(ax.p(80,160))
    --[[
    local bezier2 = BezierConfig()
    bezier2.controlPoint_1 = ax.p(100, size.height / 2)
    bezier2.controlPoint_2 = ax.p(200, - size.height / 2)
    bezier2.endPosition = ax.p(240, 160)
    ]]--
    local bezier2 ={
        ax.p(100, size.height / 2),
        ax.p(200, - size.height / 2),
        ax.p(240, 160)
    }

    local bezierTo1 = ax.BezierTo:create(2, bezier2)

    -- sprite 3
    kathia:setPosition(ax.p(400,160))
    local bezierTo2 = ax.BezierTo:create(2, bezier2)
...
local function ActionBlink()
	...
	local action1 = ax.Blink:create(2, 10)
    local action2 = ax.Blink:create(2, 5)
...
local function ActionFade()
	...
	tamara:setOpacity(0)
    local action1 = ax.FadeIn:create(1)
    local action1Back = action1:reverse()

    local action2 = ax.FadeOut:create(1)
    local action2Back = action2:reverse()

    tamara:runAction(ax.Sequence:create( action1, action1Back))
    kathia:runAction(ax.Sequence:create( action2, action2Back))
...
local function ActionTint()
    ...
    local action1 = ax.TintTo:create(2, 255, 0, 255)
    local action2 = ax.TintBy:create(2, -127, -255, -127)
    local action2Back = action2:reverse()
...
local function ActionAnimate()
    ...
    grossini:runAction(ax.Sequence:create(action, action:reverse()))

    local cache = ax.AnimationCache:getInstance()
    cache:addAnimations("animations/animations-2.plist")
    local animation2 = cache:getAnimation("dance_1")

    local action2 = ax.Animate:create(animation2)
    tamara:runAction(ax.Sequence:create(action2, action2:reverse()))

	local animation3 = animation2:clone()

    animation3:setLoops(4)

    local action3 = ax.Animate:create(animation3)
...
local function ActionSequence()
	...
	local action = ax.Sequence:create(
        ax.MoveBy:create(2, ax.p(240,0)),
        ax.RotateBy:create(2, 540))
...
local function ActionSequenceCallback1()
	local label = ax.Label:createWithTTF("callback 1 called", s_markerFeltFontPath, 16)
    label:setAnchorPoint(ax.p(0.5, 0.5))
...
local function ActionSequenceCallback2(sender)
	local label = ax.Label:createWithTTF("callback 2 called", s_markerFeltFontPath, 16)
    label:setAnchorPoint(ax.p(0.5, 0.5))
    label:setPosition(ax.p(size.width / 4 * 2, size.height / 2))

    actionSequenceLayer:addChild(label)
end

local function ActionSequenceCallback3(sender)
	local label = ax.Label:createWithTTF("callback 3 called", s_markerFeltFontPath, 16)
    label:setAnchorPoint(ax.p(0.5, 0.5))
    label:setPosition(ax.p(size.width / 4 * 3, size.height / 2))
...
local function ActionSequence2()
    ...
    grossini:setVisible(false)
    local action = ax.Sequence:create(ax.Place:create(ax.p(200,200)),ax.Show:create(),ax.MoveBy:create(1, ax.p(100,0)), ax.CallFunc:create(ActionSequenceCallback1),ax.CallFunc:create(ActionSequenceCallback2),ax.CallFunc:create(ActionSequenceCallback3))
...
local function ActionSpawn()
	...
	local action = ax.Spawn:create(
		ax.JumpBy:create(2, ax.p(300,0), 50, 4),
		ax.RotateBy:create( 2,  720))
...
local function ActionReverse()
	...
	local jump = ax.JumpBy:create(2, ax.p(300,0), 50, 4)
    local action = ax.Sequence:create(jump, jump:reverse())
...
local function ActionDelaytime()
	...
	local move = ax.MoveBy:create(1, ax.p(150,0))
	local action = ax.Sequence:create(move, ax.DelayTime:create(2), move)
...
local function ActionRepeat()
    ...
    local a1 = ax.MoveBy:create(1, ax.p(150,0))
    local action1 = ax.Repeat:create(ax.Sequence:create(ax.Place:create(ax.p(60,60)), a1), 3)

    local a2 = ax.MoveBy:create(1, ax.p(150,0))
    local action2 = ax.RepeatForever:create(ax.Sequence:create(a2, a1:reverse()))
...
local function repeatForever(sender)
	local repeatAction = ax.RepeatForever:create(ax.RotateBy:create(1.0, 360))
...
local function ActionRepeatForever()
    ...
    local action = ax.Sequence:create(
        ...
        ax.CallFunc:create(repeatForever) )
...
local function ActionRotateToRepeat()
    ...
    local act1 = ax.RotateTo:create(1, 90)
    local act2 = ax.RotateTo:create(1, 0)
    local seq  = ax.Sequence:create(act1, act2)
    local rep1 = ax.RepeatForever:create(seq)
    local rep2 = ax.Repeat:create(seq:clone(), 10)
...
local function ActionRotateJerk()
    ...
    local seq = ax.Sequence:create(
        ax.RotateTo:create(0.5, -20),
        ax.RotateTo:create(0.5, 20))

    local rep1 = ax.Repeat:create(seq, 10)

    local seq2 = ax.Sequence:create(
        ax.RotateTo:create(0.5, -20),
        ax.RotateTo:create(0.5, 20))

    local rep2 = ax.RepeatForever:create(seq2)
...
local function ActionCallFunc()
	...
	local action = ax.Sequence:create(
        ax.MoveBy:create(2, ax.p(200,0)),
        ax.CallFunc:create(CallFucnCallback1) )
    local action2 = ax.Sequence:create(ax.ScaleBy:create(2, 2),ax.FadeOut:create(2),ax.CallFunc:create(CallFucnCallback2))
    local action3 = ax.Sequence:create(ax.RotateBy:create(3 , 360),ax.FadeOut:create(2),ax.CallFunc:create(CallFucnCallback3))
...
local function ActionCallFuncND()
    ...
    local action = ax.Sequence:create(
        ax.MoveBy:create(2, ax.p(200,0)),
        ax.CallFunc:create(doRemoveFromParentAndCleanup,{true}))
...
local function ActionReverseSequence()
	...
	local move1  = ax.MoveBy:create(1, ax.p(250,0))
    local move2  = ax.MoveBy:create(1, ax.p(0,50))
    local seq    = ax.Sequence:create(move1, move2, move1:reverse())
    local action = ax.Sequence:create(seq, seq:reverse())
...
local function ActionReverseSequence2()
	...
	local move1  = ax.MoveBy:create(1, ax.p(250,0))
    local move2  = ax.MoveBy:create(1, ax.p(0,50))
	local tog1 = ax.ToggleVisibility:create()
    local tog2 = ax.ToggleVisibility:create()
    local seq = ax.Sequence:create(move1, tog1, move2, tog2, move1:reverse())
    local action = ax.Repeat:create(ax.Sequence:create(seq, seq:reverse()), 3)

    -- Test:
    -- Also test that the reverse of Hide is Show, and vice-versa
    kathia:runAction(action)

    local move_tamara = ax.MoveBy:create(1, ax.p(100,0))
    local move_tamara2 = ax.MoveBy:create(1, ax.p(50,0))
    local hide = ax.Hide:create()

    local seq_tamara = ax.Sequence:create(move_tamara, hide, move_tamara2)
    local seq_back = seq_tamara:reverse()
    tamara:runAction(ax.Sequence:create(seq_tamara, seq_back))
...
local function ActionOrbit()
	...
	local orbit1 = ax.OrbitCamera:create(2,1, 0, 0, 180, 0, 0)
    local action1 = ax.Sequence:create(orbit1, orbit1:reverse())

    local orbit2 = ax.OrbitCamera:create(2,1, 0, 0, 180, -45, 0)
    local action2 = ax.Sequence:create(orbit2, orbit2:reverse())

    local orbit3 = ax.OrbitCamera:create(2,1, 0, 0, 180, 90, 0)
    local action3 = ax.Sequence:create(orbit3, orbit3:reverse())

    kathia:runAction(ax.RepeatForever:create(action1))
    tamara:runAction(ax.RepeatForever:create(action2))
    grossini:runAction(ax.RepeatForever:create(action3))

    local move = ax.MoveBy:create(3, ax.p(100,-100))
    local move_back = move:reverse()
    local seq = ax.Sequence:create(move, move_back)
    local rfe = ax.RepeatForever:create(seq)
...
local function ActionFollow()
	...
	grossini:setPosition(ax.p(-200, size.height / 2))
    local move = ax.MoveBy:create(2, ax.p(size.width * 3, 0))
    local move_back = move:reverse()
    local seq = ax.Sequence:create(move, move_back)
    local rep = ax.RepeatForever:create(seq)
...
local function ActionTargeted()
	...
	local jump1 = ax.JumpBy:create(2, ax.p(0, 0), 100, 3)
    local jump2 = ax.JumpBy:create(2, ax.p(0, 0), 100, 3)
    local rot1  = ax.RotateBy:create(1, 360)
    local rot2  = ax.RotateBy:create(1, 360)

    local t1 = ax.TargetedAction:create(kathia, jump2)
    local t2 = ax.TargetedAction:create(kathia, rot2)
    local seq = ax.Sequence:create(jump1, t1, rot1, t2)
    local always = ax.RepeatForever:create(seq)
...
local function addSprite(dt)
	...
	spriteTmp:setPosition(ax.p(250, 250))
...
local function Issue1305_onEnterOrExit(tag)
	local scheduler = ax.Director:getInstance():getScheduler()
	if tag == "enter" then
        spriteTmp = ax.Sprite:create("Images/grossini.png")
        spriteTmp:runAction(ax.CallFunc:create(Issue1305_log))
        spriteTmp:retain()
...
local function ActionIssue1305()
	Issue1305_layer = ax.Layer:create()
...
local function ActionIssue1305_2()
    ...
    local spr = ax.Sprite:create("Images/grossini.png")
    spr:setPosition(ax.p(200,200))
    layer:addChild(spr)

    local act1 = ax.MoveBy:create(2 ,ax.p(0, 100))
    local act2 = ax.CallFunc:create(Issue1305_2_log1)
    local act3 = ax.MoveBy:create(2, ax.p(0, -100))
    local act4 = ax.CallFunc:create(Issue1305_2_log2)
    local act5 = ax.MoveBy:create(2, ax.p(100, -100))
    local act6 = ax.CallFunc:create(Issue1305_2_log3)
    local act7 = ax.MoveBy:create(2, ax.p(-100, 0))
    local act8 = ax.CallFunc:create(Issue1305_2_log4)
    local actF = ax.Sequence:create(act1, act2, act3, act4, act5, act6, act7, act8)

    ax.Director:getInstance():getActionManager():addAction(actF ,spr, false)
...
local function ActionIssue1288()
    ...
    local spr = ax.Sprite:create("Images/grossini.png")
    spr:setPosition(ax.p(100, 100))
    layer:addChild(spr)

    local act1 = ax.MoveBy:create(0.5, ax.p(100, 0))
    local act2 = act1:reverse()
    local act3 = ax.Sequence:create(act1, act2)
    local act4 = ax.Repeat:create(act3, 2)
...
local function ActionIssue1288_2()
    ...
    local act1 = ax.MoveBy:create(0.5, ax.p(100, 0))
    spr:runAction(ax.Repeat:create(act1, 1))
...
local function ActionIssue1327()
    ...
    local act1 = ax.CallFunc:create(logSprRotation)
    local act2 = ax.RotateBy:create(0.25, 45)
    local act3 = ax.CallFunc:create(logSprRotation)
    local act4 = ax.RotateBy:create(0.25, 45)
    local act5 = ax.CallFunc:create(logSprRotation)
    local act6 = ax.RotateBy:create(0.25, 45)
    local act7 = ax.CallFunc:create(logSprRotation)
    local act8 = ax.RotateBy:create(0.25, 45)
    local act9 = ax.CallFunc:create(logSprRotation)
    spr:runAction(ax.Sequence:create(act1, act2, act3, act4, act5, act6, act7,act8, act9))
