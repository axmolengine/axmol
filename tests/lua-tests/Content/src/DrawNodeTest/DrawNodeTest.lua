...
local function DrawNodeMainLayer()
    ...
    local function createDrawNodeTest()
        ...
        --draw 1 point
        draw:drawPoint(ax.p(60,60), 4, ax.c4f(math.random(0,1), math.random(0,1), math.random(0,1), 1))

        --draw 4 small points
        local fourpoints = { ax.p(60,60), ax.p(70,70), ax.p(60,70), ax.p(70,60) }
        draw:drawPoints(fourpoints, 4, ax.c4f(math.random(0,1), math.random(0,1), math.random(0,1), 1))

        --draw a line
        draw:drawLine(ax.p(0,0), ax.p(size.width, size.height), ax.c4f(0,1,0,1))

        --draw a rectangle
        draw:drawRect(ax.p(23,23), ax.p(7,7), ax.c4f(1,1,0,1))

        --draw a solid polygon
        local vertices3 = { ax.p(60,160), ax.p(70,190), ax.p(100,190), ax.p(90,160) }
        draw:drawSolidPoly( vertices3, 4, ax.c4f(0,0,1,1) )

        --draw a solid rectangle
        draw:drawSolidRect(ax.p(10,10), ax.p(20,20), ax.c4f(1,1,0,1))

        --draw a solid circle
        draw:drawSolidCircle(ax.p(VisibleRect:center().x + 140 ,VisibleRect:center().y), 100, math.pi/2, 50, 1.0, 2.0, ax.c4f(1,0,0,0.2))

        --draw open random color poly
        local vertices = { ax.p(0,0), ax.p(50,50), ax.p(100,50), ax.p(100,100), ax.p(50,100) }
        draw:drawPoly( vertices, 5, false, ax.c4f(math.random(0,1), math.random(0,1), math.random(0,1), 1))

        --draw closed random color poly
        local vertices2 = { ax.p(30,130), ax.p(30,230), ax.p(50,200) }
        draw:drawPoly( vertices2, 3, true, ax.c4f(math.random(0,1), math.random(0,1), math.random(0,1), 1))

        --draw two circle
        draw:drawCircle(ax.p(VisibleRect:center().x + 140 ,VisibleRect:center().y), 110, math.pi/2, 50, true, 1.0, 2.0, ax.c4f(1.0, 0.0, 0.0, 0.5))

        draw:drawCircle(ax.p(VisibleRect:center().x - 140 ,VisibleRect:center().y), 50, math.pi/2, 30, false, ax.c4f(math.random(0,1), math.random(0,1), math.random(0,1), 1))

        --draw some beziers
        draw:drawQuadBezier(ax.p(size.width - 150, size.height - 150), ax.p(size.width - 70, size.height - 10), ax.p(size.width - 10, size.height - 10), 10, ax.c4f(math.random(0,1), math.random(0,1), math.random(0,1), 0.5))

        draw:drawQuadBezier(ax.p(0, size.height), ax.p(size.width/2, size.height/2), ax.p(size.width, size.height), 50, ax.c4f(math.random(0,1), math.random(0,1), math.random(0,1), 0.5))

        draw:drawCubicBezier(ax.p(VisibleRect:center()), ax.p(VisibleRect:center().x+30,VisibleRect:center().y+50), ax.p(VisibleRect:center().x+60,VisibleRect:center().y-50),VisibleRect:right(),100, ax.c4f(math.random(0,1), math.random(0,1), math.random(0,1), 0.5))
        ...
        local array = {
                ...
                ax.p(0, 0) }
        draw:drawCardinalSpline(array, 0.5, 50, ax.c4f(math.random(0,1), math.random(0,1), math.random(0,1), 0.5))
        ...
        local array2 = {
                ...
                ax.p(size.width / 2, 30) }
        draw:drawCatmullRom(array2, 50, ax.c4f(math.random(0,1), math.random(0,1), math.random(0,1), 0.5))

        --Draw 10 circles
        for i=1, 10 do
            draw:drawDot(ax.p(size.width/2, size.height/2), 10*(10-i), ax.c4f(math.random(0,1), math.random(0,1), math.random(0,1), 0.5))
        end

        --Draw polygons
        points = { ax.p(size.height/4, 0), ax.p(size.width, size.height / 5), ax.p(size.width / 3 * 2, size.height) }
        draw:drawPolygon(points, #(points), ax.c4f(1,0,0,0.5), 4, ax.c4f(0,0,1,1))
        ...
        draw:drawPolygon(star1, #(star1), ax.c4f(1,0,0,0.5), 1, ax.c4f(0,0,1,1))
        ...
        draw:drawPolygon(star2, #(star2), ax.c4f(1,0,0,0.5), 1, ax.c4f(0,0,1,1))

        draw:drawSegment(ax.p(20,size.height), ax.p(20,size.height/2), 10, ax.c4f(0, 1, 0, 1))

        draw:drawSegment(ax.p(10,size.height/2), ax.p(size.width/2, size.height/2), 40, ax.c4f(1, 0, 1, 0.5))
