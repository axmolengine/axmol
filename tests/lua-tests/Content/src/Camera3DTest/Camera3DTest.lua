...
function Camera3DTestDemo:addNewSpriteWithCoords(vec3, filename, playAnimation, scale, bindCamera)
    ...
    if nil ~= playAnimation then
        local animation = ax.Animation3D:create(filename,"Take 001")
...
function Camera3DTestDemo:SwitchViewCallback(sender, value)
    ...
    if self._cameraType == CameraType.FreeCamera then
        ...
        self._camera:setPosition3D(ax.vec3add(ax.vec3(0, 130, 130), pos3D))
        self._camera:lookAt(self._sprite3D:getPosition3D(), ax.vec3(0,1,0))
    ...
    elseif self._cameraType == CameraType.FirstCamera then
        ...
        local pos3D = self._sprite3D:getPosition3D()
        self._camera:lookAt(ax.vec3add(pos3D, ax.vec3mul(newFaceDir, 50)), ax.vec3(0, 1, 0))
        self._incRot:setEnabled(true)
        self._decRot:setEnabled(true)
    elseif self._cameraType == CameraType.ThirdCamera then
        local pos3D = self._sprite3D:getPosition3D()
        self._camera:setPosition3D(ax.vec3(pos3D.x, pos3D.y + 130, pos3D.z + 130))
        self._camera:lookAt(self._sprite3D:getPosition3D(), ax.vec3(0, 1, 0))
...
function Camera3DTestDemo:updateState( dt )
    if self._sprite3D ~= nil then
        ...
        curFaceDir = ax.vec3normalize(curFaceDir)
...
function Camera3DTestDemo:onEnter()
        ...
        if event ~= nil then
            ...
            if self._cameraType == CameraType.FreeCamera or self._cameraType == CameraType.FirstCamera then
                ...
                if nil ~= self._sprite3D and self._cameraType == CameraType.FirstCamera then
                    self._sprite3D:setPosition3D(ax.vec3(self._camera:getPositionX(), 0, self._camera:getPositionZ()))
    ...
    self:addNewSpriteWithCoords(ax.vec3(0, 0, 0), "MeshRendererTest/girl.c3b", true, 0.2, true)

    local ttfConfig = {}
    ttfConfig.fontFilePath = "fonts/arial.ttf"
    ttfConfig.fontSize = 20

    local label1 = ax.Label:createWithTTF(ttfConfig,"zoom out")
    ...
    menuItem1:registerScriptTapHandler(function (tag, sender ) -- zoom out
        if nil ~= self._camera and self._cameraType ~= CameraType.FirstCamera then
            local cameraPos = self._camera:getPosition3D()
            local cameraPosNormalize = ax.vec3normalize(cameraPos)
    ...
    local label2 = ax.Label:createWithTTF(ttfConfig,"zoom in")
    ...
    local label3 = ax.Label:createWithTTF(ttfConfig,"rotate+")
    local menuItem3 = ax.MenuItemLabel:create(label3)
    ...
    local label4 = ax.Label:createWithTTF(ttfConfig,"rotate-")
    local menuItem4 = ax.MenuItemLabel:create(label4)
    ...
    local label5 = ax.Label:createWithTTF(ttfConfig,"free")
    local menuItem5 = ax.MenuItemLabel:create(label5)
    ...
    local label6 = ax.Label:createWithTTF(ttfConfig,"third person")
    local menuItem6 = ax.MenuItemLabel:create(label6)
    ...
    local label7 = ax.Label:createWithTTF(ttfConfig,"first person")
    local menuItem7 = ax.MenuItemLabel:create(label7)
    menuItem7:registerScriptTapHandler(function (tag, sender ) -- first person
        ...
        local newFaceDir = ax.vec3normalize({x = -transformMat[9],y = -transformMat[10], z = -transformMat[11]})
        local pos3D = self._sprite3D:getPosition3D()
        self._camera:setPosition3D(ax.vec3(pos3D.x, pos3D.y + 35, pos3D.z))
    ...
    local menu = ax.Menu:create(menuItem1,menuItem2,menuItem3,menuItem4,menuItem5,menuItem6,menuItem7)
    ...
    if self._camera == nil then
        self._camera = ax.Camera:createPerspective(60, s.width / s.height, 1, 1000)
...
function CameraRotationTest:onEnter()
    ...
    camNode = ax.Node:create()
    camNode:setPositionZ(ax.Camera:getDefaultCamera():getPosition3D().z)
    camControlNode:addChild(camNode)

    local sp3d = ax.Sprite3D:create()
    sp3d:setPosition(s.width/2, s.height/2)
    self:addChild(sp3d)

    local lship = ax.Label:create()
    ...
    local p1 = ax.ParticleSystemQuad:create("Particles/SmallSun.plist")
    p1:setPosition(30,80)
    bill1:addChild(p1)

    bill2 = ax.BillBoard:create("Images/Icon.png")
    bill2:setPosition3D(ax.vec3(s.width/2 - 50, s.height/2 - 10, 10))
    bill2:setScale(0.6)
    self:addChild(bill2)
    ...
    local p2 = ax.ParticleSystemQuad:create("Particles/SmallSun.plist")
    p2:setPosition(30,80)
    bill2:addChild(p2)

    --3D models
    local model = ax.Sprite3D:create("MeshRendererTest/boss1.obj")
    model:setScale(4)
    model:setTexture("MeshRendererTest/boss.png")
    model:setPosition3D(ax.vec3(s.width/2, s.height/2, 0))
    self:addChild(model)

    --Listener
    lis = ax.PointerEventListener:create()
    lis:registerScriptHandler(function(event)
        return true
    end,ax.Handler.EVENT_POINTER_DOWN )
    ...
    lis:registerScriptHandler(function(event)
        ...
        local dx = event:getDelta().x
        local rot = camControlNode:getRotation3D()
        rot.y = rot.y + dx
        camControlNode:setRotation3D(rot)

        local worldPos = ax.vec3(0.0, 0.0, 0.0)
        local decompose = ax.mat4.new(camNode:getNodeToWorldTransform()):decompose(nil, nil, worldPos)
...
function FogTestDemo:createMenu()
    ...
    local label1 = ax.Label:createWithTTF(ttfConfig,"Linear ")
    local menuItem1 = ax.MenuItemLabel:create(label1)
    menuItem1:registerScriptTapHandler(function (tag, sender )
        self._shader1:setUniform("u_fogColor", ax.bytearray.from_vec4( ax.vec4(0.5,0.5,0.5,1.0)))
        self._shader1:setUniform("u_fogStart",ax.bytearray.from_float( 10))
        self._shader1:setUniform("u_fogEnd",ax.bytearray.from_float( 60))
        self._shader1:setUniform("u_fogEquation" ,ax.bytearray.from_int(0))

        self._shader2:setUniform("u_fogColor", ax.bytearray.from_vec4( ax.vec4(0.5,0.5,0.5,1.0)))
        self._shader2:setUniform("u_fogStart",ax.bytearray.from_float( 10))
        self._shader2:setUniform("u_fogEnd",ax.bytearray.from_float( 60))
        self._shader2:setUniform("u_fogEquation" ,ax.bytearray.from_int(0))
    ...
    local label2 = ax.Label:createWithTTF(ttfConfig,"Exp")
    local menuItem2 = ax.MenuItemLabel:create(label2)
    menuItem2:registerScriptTapHandler(function (tag, sender )
        self._shader1:setUniform("u_fogColor", ax.bytearray.from_vec4( ax.vec4(0.5,0.5,0.5,1.0)))
        self._shader1:setUniform("u_fogDensity",ax.bytearray.from_float( 0.03))
        self._shader1:setUniform("u_fogEquation" ,ax.bytearray.from_int(1))

        self._shader2:setUniform("u_fogColor", ax.bytearray.from_vec4( ax.vec4(0.5,0.5,0.5,1.0)))
        self._shader2:setUniform("u_fogDensity",ax.bytearray.from_float( 0.03))
        self._shader2:setUniform("u_fogEquation" ,ax.bytearray.from_int(1))

        self._sprite3D1:setProgramState(self._shader1)
        self._sprite3D2:setProgramState(self._shader2)
    end)
    local label3 = ax.Label:createWithTTF(ttfConfig,"Exp2")
    local menuItem3 = ax.MenuItemLabel:create(label3)
    menuItem3:registerScriptTapHandler(function (tag, sender )
        self._shader1:setUniform("u_fogColor", ax.bytearray.from_vec4( ax.vec4(0.5,0.5,0.5,1.0)))
        self._shader1:setUniform("u_fogDensity",ax.bytearray.from_float( 0.03))
        self._shader1:setUniform("u_fogEquation" ,ax.bytearray.from_int(2))

        self._shader2:setUniform("u_fogColor", ax.bytearray.from_vec4( ax.vec4(0.5,0.5,0.5,1.0)))
        self._shader2:setUniform("u_fogDensity",ax.bytearray.from_float( 0.03))
        self._shader2:setUniform("u_fogEquation" ,ax.bytearray.from_int(2))

        self._sprite3D1:setProgramState(self._shader1)
        self._sprite3D2:setProgramState(self._shader2)
    end)
    local menu = ax.Menu:create(menuItem1, menuItem2, menuItem3)
...
function FogTestDemo:createLayer3D()
    ...
    self._sprite3D2 = ax.Sprite3D:create("MeshRendererTest/teapot.c3b")
    ...
    self._sprite3D1:setPosition3D( ax.vec3( 0, 0,0 ) )
    self._sprite3D1:setScale(2.0)
    self._sprite3D1:setRotation3D(ax.vec3(-90,180,0))

    self._layer3D:addChild(self._sprite3D2)
    self._sprite3D2:setPosition3D( ax.vec3( 0, 0,-20 ) )
    self._sprite3D2:setScale(2.0)
    self._sprite3D2:setRotation3D(ax.vec3(-90,180,0))

    if self._camera == nil then
        self._camera = ax.Camera:createPerspective(60, s.width/s.height, 1, 1000)
        self._camera:setCameraFlag(ax.CameraFlag.USER1)
        self._camera:setPosition3D(ax.vec3(0, 30, 40))
        self._camera:lookAt(ax.vec3(0,0,0), ax.vec3(0, 1, 0))
...
function CameraArcBallDemo:calculateArcBall(axis, angle, p1x, p1y, p2x, p2y)
    local rotation_matrix = ax.mat4.createRotation(self._rotationQuat)
    --rotation y
    ...
    local p1 = ax.vec3sub(ax.vec3add(ax.vec3mul(sv, p1x), ax.vec3mul(uv, p1y)), ax.vec3mul(lv, projectZ1))
    --end point screen transform to 3d
    local projectZ2 = self:projectToSphere(self._radius, p2x, p2y)
    local p2 = ax.vec3sub(ax.vec3add(ax.vec3mul(sv, p2x), ax.vec3mul(uv, p2y)), ax.vec3mul(lv, projectZ2))
    --calculate rotation axis
    axis = vec3_cross(p2, p1, axis)
    axis = ax.vec3normalize(axis)

    local pdiff = ax.vec3sub(p2, p1)
    local t = math.sqrt(ax.vec3dot(pdiff, pdiff)) / (2.0 * self._radius)
    --clamp -1 to 1
    if t > 1.0 then
        t = 1.0
    end
    ...
    listener:registerScriptHandler(function(event)
        if event ~= nil then
            if self._operate == OperateCamType.RotateCamera then
                ...
                local quat = ax.quaternion(normal.x * sinHalfAngle, normal.y * sinHalfAngle, normal.z * sinHalfAngle, math.cos(math.deg(halfAngle)))
                ...
                self._rotationQuat = ax.quaternion(x, y, z, w)
...
function CameraArcBallDemo:createLayer3D()
    ...
    if self._camera == nil then
        self._camera = ax.Camera:createPerspective(60, s.width/s.height, 1, 1000)
        self._camera:setCameraFlag(ax.CameraFlag.USER1)
        self._camera:setPosition3D(ax.vec3(0, 10, 50))
        self._camera:lookAt(ax.vec3(0, 0, 0), ax.vec3(0, 1, 0))
        self._layer3D:addChild(self._camera)
    end

    self._sprite3D1 = ax.Sprite3D:create("MeshRendererTest/orc.c3b")
    self._sprite3D1:setScale(0.5)
    self._sprite3D1:setRotation3D(ax.vec3(0,180,0))
    self._sprite3D1:setPosition3D(ax.vec3(0,0,0))
    self._layer3D:addChild(self._sprite3D1)

    self._sprite3D2 = ax.Sprite3D:create("MeshRendererTest/boss.c3b")
    self._sprite3D2:setScale(0.6)
    self._sprite3D2:setRotation3D(ax.vec3(-90,0,0))
    self._sprite3D2:setPosition3D(ax.vec3(20,0,0))
    self._layer3D:addChild(self._sprite3D2)
...
function CameraArcBallDemo:updateCameraTransform()
    -- body
    local trans = ax.mat4.createTranslation(ax.vec3(0.0, 10.0, self._distanceZ))
    local rot = ax.mat4.new(ax.mat4.createRotation(self._rotationQuat))
    local center = ax.mat4.new(ax.mat4.createTranslation(self._center))
    local result = ax.mat4.new(center:multiply(rot)):multiply(trans)

    self._camera:setNodeToParentTransform(result)
end

function CameraArcBallDemo:onEnter()
    self._rotationQuat = ax.quaternion(0.0, 0.0, 0.0, 1.0)
    self:setEventListener()
    self:createLayer3D()
end
