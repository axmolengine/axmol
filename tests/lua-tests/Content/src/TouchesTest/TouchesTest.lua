...
local function resetAndScoreBallForPlayer(player)
    m_ballStartingVelocity = ax.pMul(m_ballStartingVelocity, -1.1);
...
local function doStep(delta)
    ...
    local ballPosition = ax.p(m_ball:getPosition())
...
local function CreateTouchesLayer()
    ...
    m_ballStartingVelocity = ax.p(20.0, -100.0);
    local mgr = ax.Director:getInstance():getTextureCache()
    ...
    local paddleTexture = ax.Director:getInstance():getTextureCache():addImage(s_Paddle);
    ...
    paddle:setPosition( ax.p(VisibleRect:center().x, nextPaddlePos) )
