...
// http://msdn.microsoft.com/en-us/library/windows/desktop/ee418650(v=vs.85).aspx Note: using log2 sizes so we do not
// need to clamp (multi-operation) T is the contained type Note this is not true lockless as the use of flags as a form
// of lock state.
...
template <uint8_t cSizeLog2, typename T>
inline LockLessMultiReadPipe<cSizeLog2, T>::LockLessMultiReadPipe() : m_WriteIndex(0), m_ReadCount(0), m_ReadIndex(0)
{
...
template <uint8_t cSizeLog2, typename T>
inline bool LockLessMultiReadPipe<cSizeLog2, T>::ReaderTryReadBack(T* pOut)
{
    ...
    while (true)
    {
        ...
        bool bSuccess     = m_Flags[actualReadIndex].compare_exchange_strong(
            previous, FLAG_INVALID, std::memory_order_acq_rel, std::memory_order_relaxed);
        if (bSuccess)
        {
            break;
        }
...
template <uint8_t cSizeLog2, typename T>
inline bool LockLessMultiReadPipe<cSizeLog2, T>::WriterTryReadFront(T* pOut)
{
    uint32_t writeIndex     = m_WriteIndex.load(std::memory_order_relaxed);
    ...
    while (true)
    {
        ...
        uint32_t previous = FLAG_CAN_READ;
        bool success      = m_Flags[actualReadIndex].compare_exchange_strong(
            previous, FLAG_INVALID, std::memory_order_acq_rel, std::memory_order_relaxed);
        if (success)
        {
            break;
...
template <uint8_t cSizeLog2, typename T>
inline bool LockLessMultiReadPipe<cSizeLog2, T>::WriterTryWriteFront(const T& in)
{
