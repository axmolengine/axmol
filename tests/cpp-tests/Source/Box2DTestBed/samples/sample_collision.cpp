...
public:
    ...
    explicit ShapeDistance(Settings& settings) : Sample(settings)
    {
    ...
    static Sample* Create(Settings& settings) { return new ShapeDistance(settings); }
...
{
public:
    explicit DynamicTree(Settings& settings) : Sample(settings)
    {
        if (settings.restart == false)
        {
    ...
    void MouseMove(b2Vec2 p) override { m_endPoint = p; }
    ...
    void Step(Settings&) override
    {
        ...
        if (m_rayDrag)
        {
            ...
            g_draw.DrawString(5, m_textLine, "node visits = %d, leaf visits = %d", result.nodeVisits,
                              result.leafVisits);
            m_textLine += m_textIncrement;
        }
        ...
        g_draw.DrawString(5, m_textLine, "proxies = %d, height = %d, hmin = %d, area ratio = %.1f", m_proxyCount,
                          height, hmin, areaRatio);
        m_textLine += m_textIncrement;
    ...
    static Sample* Create(Settings& settings) { return new DynamicTree(settings); }
...
{
public:
    explicit RayCast(Settings& settings) : Sample(settings)
    {
        if (settings.restart == false)
        {
    ...
    static Sample* Create(Settings& settings) { return new RayCast(settings); }
...
public:
    ...
    explicit CastWorld(Settings& settings) : Sample(settings)
    {
        if (settings.restart == false)
        {
    ...
    void Step(Settings& settings) override
    {
        ...
        else
        {
            ...
            b2Capsule capsule     = {b2TransformPoint(transform, {-0.25f, 0.0f}),
                                     b2TransformPoint(transform, {0.25f, 0.0f}), m_castRadius};
    ...
    static Sample* Create(Settings& settings) { return new CastWorld(settings); }
...
public:
    ...
    explicit OverlapWorld(Settings& settings) : Sample(settings)
    {
        if (settings.restart == false)
        {
    ...
    static Sample* Create(Settings& settings) { return new OverlapWorld(settings); }
...
{
public:
    explicit Manifold(Settings& settings) : Sample(settings)
    {
        if (settings.restart == false)
        {
    ...
    void Step(Settings&) override
    {
        ...
        // chain-segment vs capsule
        {
            ...
            b2Manifold m1 =
                b2CollideChainSegmentAndCapsule(&segment1, transform1, &capsule, transform2, &m_smgcapCache1);
            b2Manifold m2 =
                b2CollideChainSegmentAndCapsule(&segment2, transform1, &capsule, transform2, &m_smgcapCache2);
    ...
    static Sample* Create(Settings& settings) { return new Manifold(settings); }
...
public:
    ...
    explicit SmoothManifold(Settings& settings) : Sample(settings)
    {
        if (settings.restart == false)
        {
    ...
    virtual ~SmoothManifold() override { free(m_segments); }
    ...
    static Sample* Create(Settings& settings) { return new SmoothManifold(settings); }
...
public:
    ...
    explicit ShapeCast(Settings& settings) : Sample(settings)
    {
        if (settings.restart == false)
        {
    ...
    void Step(Settings& settings) override
    {
        ...
        DrawTextLine("hit = %s, iterations = %d, lambda = %g, distance = %g", output.hit ? "true" : "false",
    ...
    static Sample* Create(Settings& settings) { return new ShapeCast(settings); }
...
{
public:
    explicit TimeOfImpact(Settings& settings) : Sample(settings)
    {
        if (settings.restart == false)
        {
    ...
    static Sample* Create(Settings& settings) { return new TimeOfImpact(settings); }
