...
static constexpr int32_t gc_TaskAlmostCompleteCount =
    1;  // GetIsComplete() will return false, but execution is done and about to complete
...
struct alignas(enki::gc_CacheLineSize) ThreadDataStore
{
    ...
    char prevent_false_Share[enki::gc_CacheLineSize - sizeof(std::atomic<ThreadState>) - sizeof(semaphoreid_t*) -
                             sizeof(uint32_t)];  // required to prevent alignment padding warning
};
...
bool TaskScheduler::RegisterExternalTaskThread()
{
    ...
    {
        for (uint32_t thread = GetNumFirstExternalTaskThread();
             thread < GetNumFirstExternalTaskThread() + m_Config.numExternalTaskThreads; ++thread)
        {
            ThreadState threadStateExpected = ENKI_THREAD_STATE_EXTERNAL_UNREGISTERED;
            if (m_pThreadDataStore[thread].threadState.compare_exchange_strong(threadStateExpected,
                                                                               ENKI_THREAD_STATE_EXTERNAL_REGISTERED))
            {
...
void TaskScheduler::DeRegisterExternalTaskThread()
{
    ...
    {
        --m_NumExternalTaskThreadsRegistered;
        m_pThreadDataStore[gtl_threadNum].threadState.store(ENKI_THREAD_STATE_EXTERNAL_UNREGISTERED,
...
void TaskScheduler::StartThreads()
{
    ...
    for (uint32_t thread = GetNumFirstExternalTaskThread();
         thread < m_Config.numExternalTaskThreads + GetNumFirstExternalTaskThread(); ++thread)
    {
    ...
    else
    {
        ...
        // ensure m_NumPartitions, m_NumInitialPartitions non zero, can happen if m_NumThreads > 1 &&
        // GetNumHardwareThreads() == 1
...
#ifdef ENKI_USE_WINDOWS_PROCESSOR_API
    // x64 bit Windows may support >64 logical processors using processor groups, and only allocate threads to a default
    // group. We need to detect this and distribute threads accordingly
    if (GetNumHardwareThreads() > 64 &&  // only have processor groups if > 64 hardware threads
        std::thread::hardware_concurrency() <
            GetNumHardwareThreads() &&  // if std::thread sees > 64 hardware threads no need to distribute
        std::thread::hardware_concurrency() <
            m_NumThreads)  // no need to distribute if number of threads requested lower than std::thread sees
    {
        ...
        if (success)
        {
            ...
            uint32_t currLogicalProcess = GetActiveProcessorCount(
                (WORD)mainProcessorGroup);  // we start iteration at end of current process group's threads

            // If more threads are created than there are logical processors then we still want to distribute them
            // evenly amongst groups so we iterate continuously around the groups until we reach m_NumThreads
            uint32_t group = 0;
            while (currLogicalProcess < m_NumThreads)
            {
                ++group;  // start at group 1 since we set currLogicalProcess to start of next group
                uint32_t currGroup = (group + mainProcessorGroup) %
                                     numProcessorGroups;  // we start at mainProcessorGroup, go round in circles
                uint32_t groupNumLogicalProcessors = GetActiveProcessorCount((WORD)currGroup);
                ENKI_ASSERT(groupNumLogicalProcessors <= 64);
                uint64_t GROUPMASK =
                    0xFFFFFFFFFFFFFFFFULL >>
                    (64 - groupNumLogicalProcessors);  // group mask should not have 1's where there are no processors
                for (uint32_t groupLogicalProcess = 0;
                     (groupLogicalProcess < groupNumLogicalProcessors) && (currLogicalProcess < m_NumThreads);
                     ++groupLogicalProcess, ++currLogicalProcess)
                {
                    if (currLogicalProcess > m_Config.numExternalTaskThreads + GetNumFirstExternalTaskThread())
                    {
                        ...
                        // If a thread is assigned to a different group than the process, the process's affinity is
                        // updated to include the thread's affinity and the process becomes a multi-group process.
...
/*  xxHash variant based on documentation on
   ...
   translations into other languages and incorporation into compilations, provided that the copyright notice and this
   notice are preserved, and that any substantive changes or deletions from the original are clearly marked.
    ...
    if (bHaveTask)
    {
        ...
        if (subTask.pTask->m_RangeToRun < partitionSize)
        {
            ...
            if (threadNum_ != threadToCheck)
            {
                ...
                rangeToSplit =
                    std::max(rangeToSplit, (subTask.partition.end - subTask.partition.start) / gc_MaxStolenPartitions);
            }
...
void TaskScheduler::TaskComplete(ICompletable* pTask_, bool bWakeThreads_, uint32_t threadNum_)
{
    ...
    while (pDependent)
    {
        ...
        int32_t prevDeps           = pDependentCurr->pTaskToRunOnCompletion->m_DependenciesCompletedCount.fetch_add(
...
void TaskScheduler::WaitForTaskCompletion(const ICompletable* pCompletable_, uint32_t threadNum_)
{
    ...
    if (gc_TaskAlmostCompleteCount >= pCompletable_->m_RunningCount.load(std::memory_order_acquire) ||
        HaveTasks(threadNum_))
    {
...
void TaskScheduler::WakeThreadsForNewTasks()
{
    ...
    while (waiting > 0 && !m_NumThreadsWaitingForNewTasks.compare_exchange_weak(waiting, 0, std::memory_order_release,
...
void TaskScheduler::AddPinnedTaskInt(IPinnedTask* pTask_)
{
    ...
    ThreadState statePinnedTaskThread =
        m_pThreadDataStore[pTask_->threadNum].threadState.load(std::memory_order_acquire);
    ...
    while (GetIsRunningInt() && (bHaveTasks || otherThreadsRunning))
    {
        ...
        if (spinCount > gc_SpinCount)
        {
            ...
            do
            {
                ...
                if (dummyWaitTask.threadNum != ourThreadNum &&
                    dummyWaitTask.threadNum > m_Config.numExternalTaskThreads)
                {
                    ThreadState state =
                        m_pThreadDataStore[dummyWaitTask.threadNum].threadState.load(std::memory_order_acquire);
...
{
    T* pRet = (T*)m_Config.customAllocator.alloc(alignof(T), num_ * sizeof(T), m_Config.customAllocator.userData, file_,
                                                 line_);
...
{
    T* pRet =
        (T*)m_Config.customAllocator.alloc(alignof(T), sizeof(T), m_Config.customAllocator.userData, file_, line_);
    return pRet;
}
