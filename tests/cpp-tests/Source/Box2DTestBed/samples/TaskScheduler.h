...
public:
    ...
    void SetDependenciesArr(D& dependencyArray_, const T (&taskArray_)[SIZE]);
    template <typename D, typename T>
    void SetDependenciesArr(D& dependencyArray_, std::initializer_list<T*> taskpList_);
    template <typename D, typename T, int SIZE>
    void SetDependenciesArr(D (&dependencyArray_)[SIZE], const T (&taskArray_)[SIZE]);
    template <typename D, typename T, int SIZE>
    void SetDependenciesArr(D (&dependencyArray_)[SIZE], std::initializer_list<T*> taskpList_);
    template <typename D, typename T, int SIZE>
    void SetDependenciesVec(D& dependencyVec_, const T (&taskArray_)[SIZE]);
    template <typename D, typename T>
    void SetDependenciesVec(D& dependencyVec_, std::initializer_list<T*> taskpList_);
...
public:
    ...
    ITaskSet(uint32_t setSize_, uint32_t minRange_)
        : m_SetSize(setSize_), m_MinRange(minRange_), m_RangeToRun(minRange_)
    {}
...
public:
    ...
    // while( !GetIsShutdownRequested() ) {} can be used in tasks which loop, to check if enkiTS has been requested to
    // shutdown. If GetIsShutdownRequested() returns true should then exit. Not required for finite tasks Safe to use
    // with WaitforAllAndShutdown() and ShutdownNow() where this will be set Not safe to use with WaitforAll(), use
    // GetIsWaitforAllCalled() instead.
    ...
    // This is also set when the task manager is shutting down, so no need to have an additional check for
    ...
    ENKITS_API void WaitforTask(const ICompletable* pCompletable_,
                                enki::TaskPriority priorityOfLowestToRun_ = TaskPriority(TASK_PRIORITY_NUM - 1));
    ...
    // and NO_THREAD_NUM for all other non-enkiTS threads which have not been registered ( see
    // RegisterExternalTaskThread() ), and < GetNumTaskThreads() for all registered and internal enkiTS threads. It is
    // guaranteed that GetThreadNum() < GetNumTaskThreads() unless it is NO_THREAD_NUM
...
private:
    ...
    template <typename T>
    T* NewArray(size_t num_, const char* file_, int line_);
    template <typename T>
    void DeleteArray(T* p_, size_t num_, const char* file_, int line_);
    template <class T, class... Args>
    T* New(const char* file_, int line_, Args&&... args_);
    template <typename T>
    void Delete(T* p_, const char* file_, int line_);
    template <typename T>
    T* Alloc(const char* file_, int line_);
    template <typename T>
    void Free(T* p_, const char* file_, int line_);
