...
void Sample::Step(Settings& settings)
{
    ...
    if (settings.drawCounters)
    {
        ...
        DrawTextLine("bodies/shapes/contacts/joints = %d/%d/%d/%d", s.bodyCount, s.shapeCount, s.contactCount,
                     s.jointCount);
    ...
    if (settings.drawProfile)
    {
        ...
        DrawTextLine("collide [ave] (max) = %5.2f [%6.2f] (%6.2f)", p.collide, aveProfile.collide,
        ...
        DrawTextLine("> solve constraints [ave] (max) = %5.2f [%6.2f] (%6.2f)", p.solveConstraints,
        ...
        DrawTextLine(">> apply restitution [ave] (max) = %5.2f [%6.2f] (%6.2f)", p.applyRestitution,
        ...
        DrawTextLine("> bullets [ave] (max) = %5.2f [%6.2f] (%6.2f)", p.bullets, aveProfile.bullets,
                     m_maxProfile.bullets);
        DrawTextLine("sensors [ave] (max) = %5.2f [%6.2f] (%6.2f)", p.sensors, aveProfile.sensors,
                     m_maxProfile.sensors);
    }
}
...
int Sample::ParsePath(const char* svgPath, b2Vec2 offset, b2Vec2* points, int capacity, float scale, bool reverseOrder)
{
    ...
    while (*ptr != '\0')
    {
        if (isdigit(*ptr) == 0 && *ptr != '-')
        {
            ...
            if (command == 'M' || command == 'L' || command == 'H' || command == 'V' || command == 'm' ||
