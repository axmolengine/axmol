...
{
public:
    explicit LargeWorld(Settings& settings) : Sample(settings)
    {
        m_period     = 40.0f;
...

        {
            ...
            for (int i = 0; i < m_gridCount; ++i)
            {
                ...
                {
                    b2Polygon square =
                        b2MakeOffsetBox(0.4f * m_gridSize, 0.4f * m_gridSize, {xShape, y}, b2Rot_identity);
                    square.radius = 0.1f;
    ...
    static Sample* Create(Settings& settings) { return new LargeWorld(settings); }
