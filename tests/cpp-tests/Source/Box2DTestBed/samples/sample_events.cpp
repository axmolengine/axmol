...
public:
    ...
    explicit SensorFunnel(Settings& settings) : Sample(settings)
    {
...

        {
            ...
            //{124.354, 293.688}, {95.250, 293.688},	{95.250, 261.938},	{42.333, 222.250},	{42.333,
            //193.146}, {76.729, 193.146},	{42.333, 171.979},	{42.333, 121.708},	{76.729, 121.708},
            //{42.333, 100.542},
            //};
    ...
    void CreateElement()
    {
        ...
        else
        {
            ...
            bool colorize       = true;
            CreateHuman(human, m_worldId, center, scale, jointFriction, jointHertz, jointDamping, index + 1, human,
                        colorize);
    ...
    static Sample* Create(Settings& settings) { return new SensorFunnel(settings); }
...
{
public:
    explicit SensorBookend(Settings& settings) : Sample(settings)
    {
        if (settings.restart == false)
        {
    ...
    static Sample* Create(Settings& settings) { return new SensorBookend(settings); }
...
public:
    ...
    explicit FootSensor(Settings& settings) : Sample(settings)
    {
        if (settings.restart == false)
        {
            g_camera.m_center = {0.0f, 6.0f};
    ...
    static Sample* Create(Settings& settings) { return new FootSensor(settings); }
...
public:
    ...
    explicit ContactEvent(Settings& settings) : Sample(settings)
    {
        if (settings.restart == false)
        {
            g_camera.m_center = {0.0f, 0.0f};
    ...
    void Step(Settings& settings) override
    {
        ...
        for (int i = 0; i < contactEvents.beginCount; ++i)
        {
            ...
            // The begin touch events have the contact manifolds, but the impulses are zero. This is because the
            // manifolds are gathered before the contact solver is run.

            // We can get the final contact data from the shapes. The manifold is shared by the two shapes, so we just
            // need the contact data from one of the shapes. Choose the one with the smallest number of contacts.
                ...
                for (int j = 0; j < countA; ++j)
                {
                    ...
                    if (B2_ID_EQUALS(idA, event.shapeIdB) || B2_ID_EQUALS(idB, event.shapeIdB))
                    {
                        ...
                        for (int k = 0; k < manifold.pointCount; ++k)
                        {
                            ...
                            g_draw.DrawSegment(point.point, point.point + point.totalNormalImpulse * normal,
                                               b2_colorBlueViolet);
    ...
    static Sample* Create(Settings& settings) { return new ContactEvent(settings); }
...
{
public:
    explicit Platformer(Settings& settings) : Sample(settings)
    {
        if (settings.restart == false)
        {
    ...
    static Sample* Create(Settings& settings) { return new Platformer(settings); }
...
public:
    ...
    explicit BodyMove(Settings& settings) : Sample(settings)
    {
        if (settings.restart == false)
        {
    ...
    static Sample* Create(Settings& settings) { return new BodyMove(settings); }
...
public:
    ...
    explicit SensorTypes(Settings& settings) : Sample(settings)
    {
        if (settings.restart == false)
        {
    ...
    static Sample* Create(Settings& settings) { return new SensorTypes(settings); }
