...
{
public:
    explicit BodyType(Settings& settings) : Sample(settings)
    {
    ...
    static Sample* Create(Settings& settings) { return new BodyType(settings); }
...
{
public:
    explicit Weeble(Settings& settings) : Sample(settings)
    {
        if (settings.restart == false)
        {
    ...
    static Sample* Create(Settings& settings) { return new Weeble(settings); }
...
{
public:
    explicit Sleep(Settings& settings) : Sample(settings)
    {
        if (settings.restart == false)
        {
    ...
    static Sample* Create(Settings& settings) { return new Sleep(settings); }
...
{
public:
    explicit BadBody(Settings& settings) : Sample(settings)
    {
        if (settings.restart == false)
        {
            g_camera.m_center = {2.3f, 10.0f};
    ...
    void Step(Settings& settings) override
    {
        ...
        g_draw.DrawString(5, m_textLine,
                          "A bad body is a dynamic body with no mass and behaves like a kinematic body.");
        m_textLine += m_textIncrement;

        g_draw.DrawString(5, m_textLine,
                          "Bad bodies are considered invalid and a user bug. Behavior is not guaranteed.");
        m_textLine += m_textIncrement;
    ...
    static Sample* Create(Settings& settings) { return new BadBody(settings); }
...
{
public:
    explicit Pivot(Settings& settings) : Sample(settings)
    {
        if (settings.restart == false)
        {
            g_camera.m_center = {0.8f, 6.4f};
    ...
    static Sample* Create(Settings& settings) { return new Pivot(settings); }
...
{
public:
    explicit Kinematic(Settings& settings) : Sample(settings)
    {
        if (settings.restart == false)
        {
    ...
    static Sample* Create(Settings& settings) { return new Kinematic(settings); }
