...
#if AXSLC_UV_TOP
    float2 fragCoord = float2(input.gl_FragCoord.x, resolution.y - input.gl_FragCoord.y);
#else
    float2 fragCoord = input.gl_FragCoord.xy;
#endif
    float2 texCoord = fragCoord / resolution;
    float2 uv = texCoord - 0.5;

    float3 mouse;
    mouse.z = 0.5;
    mouse.x = sin(u_Time[1]) * 0.5;
    mouse.y = sin(u_Time[1] * 0.913) * 0.5;

    float3 color = float3(1.4, 1.2, 1.0) * lensflare(uv, mouse.xy);
    color -= noise(texCoord * resolution) * 0.015;
