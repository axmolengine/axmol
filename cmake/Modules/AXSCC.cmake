...
# #############################################################
# #  enable shader lang by shader compiler: axscc
macro(axscc_option variable value)
  if(NOT DEFINED ${variable})
    set(${variable} ${value} CACHE STATIC "" FORCE)
  endif()
endmacro()

axscc_option(AXSCC_FRAG_SOURCE_FILE_EXTENSIONS ".frag;.fsh;.fs")
axscc_option(AXSCC_VERT_SOURCE_FILE_EXTENSIONS ".vert;.vsh;.vs")
axscc_option(AXSCC_OUT_DIR ${CMAKE_BINARY_DIR}/runtime/axsc)
axscc_option(AXSCC_FIND_PROG_ROOT "")

find_program(AXSCC_EXE NAMES axscc
  PATHS ${AXSCC_FIND_PROG_ROOT}
)

if(NOT AXSCC_EXE)
  message(STATUS, "axscc not found.")
  message(FATAL_ERROR "Please run setup.ps1 again to download axscc, and run CMake again.")
endif()

message(STATUS "AXSCC_OUT_DIR=${AXSCC_OUT_DIR}")
message(STATUS "AXSCC_FIND_PROG_ROOT=${AXSCC_FIND_PROG_ROOT}")
message(STATUS "AXSCC_FRAG_SOURCE_FILE_EXTENSIONS=${AXSCC_FRAG_SOURCE_FILE_EXTENSIONS}")
message(STATUS "AXSCC_VERT_SOURCE_FILE_EXTENSIONS=${AXSCC_VERT_SOURCE_FILE_EXTENSIONS}")

# PROPERTY: include direcotries (optional)
define_property(SOURCE PROPERTY AXSCC_INCLUDE_DIRS
...
define_property(SOURCE PROPERTY AXSCC_DEFINES
...
define_property(SOURCE PROPERTY AXSCC_OUTPUT1
...
define_property(SOURCE PROPERTY AXSCC_OUTPUT
...
function(ax_find_shaders dir varName)
  ...
  foreach(fileext ${AXSCC_FRAG_SOURCE_FILE_EXTENSIONS})
    list(APPEND SC_FILTERS "${dir}/*${fileext}")
  endforeach()

  foreach(fileext ${AXSCC_VERT_SOURCE_FILE_EXTENSIONS})
    list(APPEND SC_FILTERS "${dir}/*${fileext}")
  endforeach()
...
# This function allow make shader files (.frag, .vert) compiled with axscc
# usage:
# - ax_target_compile_shaders(axmol FILES source_files): output compiled shader to ${CMAKE_BINARY_DIR}/runtime/axsc/xxx_fs
# - ax_target_compile_shaders(axmol FILES source_files CUSTOM): output compiled shader to ${CMAKE_BINARY_DIR}/runtime/axsc/custom/xxx_fs
...
# - AXSCC_FRAG_SOURCE_FILE_EXTENSIONS: default is .frag;.fsh
# - AXSCC_VERT_SOURCE_FILE_EXTENSIONS: default is .vert;.vsh
#
function(ax_target_compile_shaders target_name)
  ...
  foreach(SC_FILE ${opt_FILES})
    ...
    # defines
    get_source_file_property(SOURCE_SC_DEFINES ${SC_FILE} AXSCC_DEFINES)
    ...
    # includes
    get_source_file_property(INC_DIRS ${SC_FILE} AXSCC_INCLUDE_DIRS)
    ...
    # input
    if(${FILE_EXT} IN_LIST AXSCC_FRAG_SOURCE_FILE_EXTENSIONS)
      list(APPEND SC_FLAGS "--frag=${SC_FILE}")
      set(SC_TYPE "fs")
    elseif(${FILE_EXT} IN_LIST AXSCC_VERT_SOURCE_FILE_EXTENSIONS)
      set(SC_TYPE "vs")
    ...
    get_source_file_property(SOURCE_SC_OUTPUT1 ${SC_FILE} AXSCC_OUTPUT1)

    string(REPLACE ";" " " FULL_COMMAND_LINE "${AXSCC_EXE};${SC_FLAGS} ...")

    if(SOURCE_SC_OUTPUT1 STREQUAL "NOTFOUND") # single output
      list(APPEND SC_FLAGS "--output=${SC_OUTPUT}")
      set_source_files_properties(${SC_FILE} DIRECTORY ${CMAKE_BINARY_DIR} PROPERTIES AXSCC_OUTPUT ${SC_OUTPUT})
      add_custom_command(
        MAIN_DEPENDENCY ${SC_FILE} OUTPUT ${SC_OUTPUT} COMMAND ${AXSCC_EXE} ${SC_FLAGS}
        COMMENT "${SC_COMMENT}"
      )
      ...
      string(REPLACE ";" " " FULL_COMMAND_LINE1 "${AXSCC_EXE};${SC_FLAGS1} ...")
      set_source_files_properties(${SC_FILE} DIRECTORY ${CMAKE_BINARY_DIR} PROPERTIES AXSCC_OUTPUT "${SC_OUTPUT};${SC_OUTPUT1}")
      add_custom_command(
        MAIN_DEPENDENCY ${SC_FILE}
        OUTPUT ${SC_OUTPUT} ${SC_OUTPUT1}
        COMMAND ${AXSCC_EXE} ${SC_FLAGS}
        COMMAND ${AXSCC_EXE} ${SC_FLAGS1}
        COMMENT "${SC_COMMENT}"
      )
...
function(ax_target_embed_compiled_shaders target_name rc_output)
  ...
  string(APPEND _filters_xml_content "  <ItemGroup Label=\"axsc\">\n")
  ...
  string(APPEND _props_xml_content "  <ItemGroup Label=\"axsc\">\n")

  foreach(shader ${opt_FILES})
    get_source_file_property(compiled_shaders ${shader} DIRECTORY ${CMAKE_BINARY_DIR} AXSCC_OUTPUT)

    if(compiled_shaders STREQUAL "NOTFOUND")
      message(FATAL_ERROR "Not found property AXSCC_OUTPUT of file: ${shader}")
    endif()

    foreach(compiled_shader ${compiled_shaders})
      file(RELATIVE_PATH compiled_shader_rp ${AXSCC_OUT_DIR} ${compiled_shader})
      file(TO_NATIVE_PATH "Content/axsc/${compiled_shader_rp}" compiled_shader_target_dir)
      file(TO_NATIVE_PATH "${compiled_shader}" compiled_shader_path)
      set(app_all_shaders_filters "${app_all_shaders_filters}  <ItemGroup Label=\"axsc\">\n")
      ...
      string(APPEND _filters_xml_content "      <Filter>Content\\axsc</Filter>\n")
  ...
  string(APPEND _filters_xml_content "    <Filter Include=\"Content\\axsc\">\n")
  ...
  set(props_file "${rt_output}/axsc.props")
  write_file("${props_file}" "${_props_xml_content}")

  set(filters_file "${rt_output}/axsc.filters.props")
